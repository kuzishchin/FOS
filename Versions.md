...
## FOS v1.0.7 build 14 08.06.2026 api-1.1
...
* Added context checking while calling API function
* Timeout mechanical is improved
* API is changed:
1. Next function are deprecated because timeout mechanism has been changed: `API_FOS_SemBinarySetTimeout`, `API_FOS_SemCntSetTimeout`.
2. Timeout is added to argument into below functions: `API_FOS_SemBinaryTake`, `API_FOS_SemCntTake`, `API_FOS_MutexTake`, `API_FOS_Queue32ReadData`, `API_FOS_Join`.
3. Arguments of the functions below are changed: `API_FOS_CreateMutex`, `API_FOS_CreateQueue32`.
4. Next function are deprecated because context watch is added: `API_FOS_SemBinaryGiveFromISR`, `API_FOS_SemCntGiveFromISR`, `API_FOS_Queue32WriteDataFromISR`, `API_FOS_SentNoteToThreadFromISR`, `API_FOS_LogDataFromISR`.
5. In `API_FOS_Yield` adde returned status.
* Also next function can be called via ISR: `API_FOS_SemBinaryTake`, `API_FOS_SemCntTake`, `API_FOS_Queue32ReadData`.
